...
def get_artisans_and_indies_shops_that_made_it(request):
    list_of_games_that_made_it = get_all_shops_that_made_it()
...
def get_all_online_shop_for_artisans_or_others(max_size_of_shop) :
    if not max_size_of_shop :
        ...
        all_for_size = Online_Shop.objects.exclude(size_of_shop = Studio.SizeInPersons.ARTISAN).exclude(size_of_shop = Publisher.SizeInPersons.INDIE)
    elif max_size_of_shop != Studio.SizeInPersons.ARTISAN :
        # No filter on publisher size for non-artisan
        all_for_size = Online_Shop.objects.filter(size_of_shop = max_size_of_shop)
    else :
        all_for_size = Online_Shop.objects.filter(size_of_shop = Studio.SizeInPersons.ARTISAN)
    
    return all_for_size

def get_all_shops_that_made_it():
    # Here we don't care about the Publisher size
    all_for_size = Online_Shop.objects.filter(Q(size_of_shop =  Studio.SizeInPersons.ARTISAN) | Q(size_of_shop =  Studio.SizeInPersons.INDIE), they_have_made_it__gt = 0)
